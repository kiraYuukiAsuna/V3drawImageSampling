...
/*
typedef unsigned char  uint8_t;   //  Datatypes for 1 byte and 2 byte pixel values
...
typedef struct
  { int      kind;
    int      width; 
    int      height;
    uint8_t   *array;   // Array of pixel values lexicographically ordered on (y,x,c).
...
#define IMAGE_PIXEL_8(img,x,y,c) \
      ((uint8_t  *) ((img)->array + (((y)*(img)->width + (x))*(img)->kind + (c))))

#define IMAGE_PIXEL_16(img,x,y,c) \
      ((uint16_t *) ((img)->array + (((y)*(img)->width + (x))*(img)->kind + (c))))
...
typedef struct
  { int      kind;
    ...
    int      height;
    int      depth;
    uint8_t   *array;   // array of pixel values lexicographically ordered on (z,y,x,c)
  } Stack;
