...
public:
    ...
    Size2D(V3DLONG s0, V3DLONG s1) {
        sz0 = s0;
        sz1 = s1;
    }

    Size2D(V3DLONG* s, V3DLONG len = 2) {
        sz0 = sz1 = 0;
        if (len >= 1) sz0 = s[0];
        if (len >= 2) sz1 = s[1];
    }
...
public:
    ...
    Size3D(V3DLONG s0, V3DLONG s1, V3DLONG s2) {
        sz0 = s0;
        sz1 = s1;
        sz2 = s2;
    }

    Size3D(V3DLONG* s, V3DLONG len = 3) {
        sz0 = sz1 = sz2 = 0;
        if (len >= 1) sz0 = s[0];
        if (len >= 2) sz1 = s[1];
        if (len >= 3) sz2 = s[2];
...
public:
    ...
    Size4D(V3DLONG s0, V3DLONG s1, V3DLONG s2, V3DLONG s3) {
        sz0 = s0;
        sz1 = s1;
        sz2 = s2;
        sz3 = s3;
    ...
    Size4D(V3DLONG* s, V3DLONG len = 4) {
        sz0 = sz1 = sz2 = sz3 = 0;
        if (len >= 1) sz0 = s[0];
        if (len >= 2) sz1 = s[1];
        if (len >= 3) sz2 = s[2];
...
public:
    ...
    Size5D(V3DLONG s0, V3DLONG s1, V3DLONG s2, V3DLONG s3, V3DLONG s4) {
        sz0 = s0;
        sz1 = s1;
        sz2 = s2;
        sz3 = s3;
    ...
    Size5D(V3DLONG* s, V3DLONG len = 4) {
        sz0 = sz1 = sz2 = sz3 = sz4 = 0;
        if (len >= 1) sz0 = s[0];
        if (len >= 2) sz1 = s[1];
        if (len >= 3) sz2 = s[2];
        if (len >= 4) sz3 = s[3];
...
public:
    ...
    bool resize(V3DLONG s0) {
        if (s0 <= 0) {
            ...
            len = 0;
            return true;
        } //make the vector empty is s0 <=0
        if (s0 > 0 && s0 == len) return true; //do nothing if the size does not change
        if (data1d) {
            delete []data1d;
            data1d = 0;
        } //else will free the existing space, and then reallocate
        V3DLONG tmplen = s0;
        data1d = new T [tmplen];
        if (data1d) {
            len = tmplen;
...
public:
    ...
    template<class Tnew>
    bool copy(Image2DSimple<Tnew>* img0, V3DLONG d0m, V3DLONG d0M, V3DLONG d1m, V3DLONG d1M) {
        if (!img0 || !img0->valid()) return false;

        V3DLONG s0m = (d0m < 0) ? 0 : d0m, s0M = (d0M < 0) ? 0 : s0M, s1m = (d1m < 0) ? 0 : d1m, s1M =
                (d1M < 0) ? 0 : d1M;
    ...
    template<class Tnew>
    bool copy(Image2DSimple<Tnew>* img0) {
        if (!img0 || !img0->valid()) return false;
    ...
    template<class Tnew>
    bool copy(Tnew** img0, V3DLONG img0sz0, V3DLONG img0sz1, V3DLONG d0m, V3DLONG d0M, V3DLONG d1m, V3DLONG d1M) {
        if (!img0) return false;

        V3DLONG s0m = (d0m < 0) ? 0 : d0m, s0M = (d0M < 0) ? 0 : d0M, s1m = (d1m < 0) ? 0 : d1m, s1M =
                (d1M < 0) ? 0 : d1M;
    ...
    template<class Tnew>
    bool plus(Image2DSimple<Tnew>* a) {
    ...
    template<class Tnew>
    bool subtract(Image2DSimple<Tnew>* a) {
        if (!valid() || !a || !a->valid()) return false;
        if (sz.sz0 != a->sz0() || sz.sz1 != a->sz1()) return false;
    ...
    template<class Tnew>
    void setValue(Tnew** img0, V3DLONG img0sz0, V3DLONG img0sz1, T value, int flag) //img0 is the mask
    {
        if (img0sz0 != sz.sz0 || img0sz1 != sz.sz1) return; //do nothing if the sizes are different
        V3DLONG i, j;
        if (flag)
        //if flag is 0, then where img0 is 0 will be set as 0; if flag=1, the  where img0 is non-zero will be set as 0.
        {
            for (j = 0; j < sz.sz1; j++) {
                for (i = 0; i < sz.sz0; i++) {
    ...
    template<class Tnew>
    void setValue(Tnew a) {
        if (!valid()) return;
        for (V3DLONG i = 0; i < len; i++) data1d[i] = (T)a;
    }

    template<class Tnew>
    bool plus_subimage(Tnew** sa, V3DLONG d0, V3DLONG d1, V3DLONG s0m, V3DLONG s0M, V3DLONG s1m, V3DLONG s1M)
    //this image plus a sub-region of a new subject
    {
    ...
    template<class Tnew>
    bool subimage_plus(V3DLONG s0m, V3DLONG s0M, V3DLONG s1m, V3DLONG s1M, Tnew** sa, V3DLONG d0, V3DLONG d1)
    //a subset region of this image plus a new subject
    {
        if (!sa || d0 <= 0 || d1 <= 0 || s0m < 0 || s0M < 0 || s0M < s0m || s1m < 0 || s1M < 0 || s1M < s1m)
            return false;
    ...
    template<class Tnew>
    bool subimage_plus_subimage(V3DLONG s0m, V3DLONG s0M, V3DLONG s1m, V3DLONG s1M, Tnew** sa, V3DLONG d0, V3DLONG d1,
                                V3DLONG d0m, V3DLONG d0M, V3DLONG d1m, V3DLONG d1M)
...
public:
    ...
    template<class T1>
    Vol3DSimple(Vol3DSimple<T1>* vsubject, V3DLONG xb, V3DLONG xe, V3DLONG yb, V3DLONG ye, V3DLONG zb, V3DLONG ze) {
        //check the validity of data
        V3DLONG vsz0 = vsubject->sz0(), vsz1 = vsubject->sz1(), vsz2 = vsubject->sz2();
        if (!vsubject->valid() || vsz0 <= 0 || vsz1 <= 0 || vsz2 <= 0) {
        ...
        if ((xb < 0 && xe < 0) || (xb >= vsz0 && xe >= vsz0)) {
            fprintf(stderr, "Invalid parameter.\n");
            return;
        }
        if ((yb < 0 && ye < 0) || (yb >= vsz1 && ye >= vsz1)) {
            fprintf(stderr, "Invalid parameter.\n");
            return;
        }
        if ((zb < 0 && ze < 0) || (zb >= vsz2 && ze >= vsz2)) {
            fprintf(stderr, "Invalid parameter.\n");
            return;
        }

        if (xb < 0) xb = 0;
        if (xb >= vsz0) xb = vsz0 - 1;
        if (xe < 0) xe = 0;
        if (xe >= vsz0) xe = vsz0 - 1;
        if (yb < 0) yb = 0;
        if (yb >= vsz1) yb = vsz1 - 1;
        if (ye < 0) ye = 0;
        if (ye >= vsz1) ye = vsz1 - 1;
        if (zb < 0) zb = 0;
        if (zb >= vsz2) zb = vsz2 - 1;
        if (ze < 0) ze = 0;
        if (ze >= vsz2) ze = vsz2 - 1;

        //allocate memory
        if (!allocateMemory(xe - xb + 1, ye - yb + 1, ze - zb + 1)) {
            fprintf(stderr, "Fail to allocate mmeory in initialize Vol3DSimple().\n");
            return;
        }
    ...
    template<class T1>
    Vol3DSimple(Vol3DSimple<T1>* vsubject) {
        //check the validity of data
        if (!vsubject->valid()) {
            fprintf(stderr, "Invalid parameter.\n");
            return;
        ...
        //allocate memory
        if (!allocateMemory(vsz0, vsz1, vsz2)) {
            fprintf(stderr, "Fail to allocate mmeory in initialize Vol3DSimple().\n");
            return;
        }
    ...
    V3DLONG maxdimsz() {
        V3DLONG tmp = (sz.sz0 > sz.sz1) ? sz.sz0 : sz.sz1;
        return (tmp > sz.sz2) ? tmp : sz.sz2;
    }
    ...
    bool padding(V3DLONG rx, V3DLONG ry, V3DLONG rz) //rx,ry, and rz are the border margin to be added
    {
        ...
        V3DLONG i, j, k;
        for (i = 0; i < len_new; i++) data1d_new[i] = (T)0;
        //note that I feel a construction function ahs to be defined for a particular class if this function is called
        for (k = 0; k < sz.sz2; k++) {
            for (j = 0; j < sz.sz1; j++) {
                for (i = 0; i < sz.sz0; i++) {
...
bool isSameSize(Image2DSimple<T1>* va, Image2DSimple<T2>* vb) //change to T1 and T2 on 080401
{
    if (!va || !va->valid() || !vb || !vb->valid()) return false;
    if (va->sz0() != vb->sz0() || va->sz1() != vb->sz1() || va->getTotalElementNumber() != vb->getTotalElementNumber())
        return false;
    return true;
}

template<class T1, class T2>
bool isSameSize(Vol3DSimple<T1>* va, Vol3DSimple<T2>* vb) //change to T1 and T2 on 070520
{
    if (!va || !va->valid() || !vb || !vb->valid()) return false;
    if (va->sz0() != vb->sz0() || va->sz1() != vb->sz1() || va->sz2() != vb->sz2() || va->getTotalElementNumber() != vb
        ->getTotalElementNumber()) return false;
    return true;
}
