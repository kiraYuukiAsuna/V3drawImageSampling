...
template<class T>
bool downsample3dvol(T*** outdata, T*** indata, V3DLONG* szin, double* dfactor, unsigned char tag) {
    V3DLONG szout[3];
    for (V3DLONG i = 0; i < 3; i++) {
        ...
        if (szout[0] <= 0) {
            fprintf(
                stderr,
                "The dfactor is not properly set, -- the downsampled size is too small. Do nothing. [%s][%d].\n",
                __FILE__, __LINE__);
            return false;
        }
    }
        ...
        for (V3DLONG k = 0; k < szout[2]; k++) {
            ...
            for (V3DLONG j = 0; j < szout[1]; j++) {
                ...
                for (V3DLONG i = 0; i < szout[0]; i++) {
                    V3DLONG i2low = (V3DLONG)(floor(i * dfactor[0])), i2high = (V3DLONG)(
                        floor((i + 1) * dfactor[0] - 1));
        ...
        for (V3DLONG k = 0; k < szout[2]; k++) {
            ...
            for (V3DLONG j = 0; j < szout[1]; j++) {
                ...
                for (V3DLONG i = 0; i < szout[0]; i++) {
                    ...
                    outdata[k][j][i] = indata[int((k2low + k2high) / 2)][int((j2low + j2high) / 2)][int(
                        (i2low + i2high) / 2)];
                }
            }
        }
...
template<class T>
bool downsample3dvol(T*&outdata, T* indata, V3DLONG* szout, V3DLONG* szin, double* dfactor, unsigned char tag) {
    //	V3DLONG szout[3];
    for (V3DLONG i = 0; i < 3; i++) {
        szout[i] = (V3DLONG)(floor(double(szin[i]) / double(dfactor[i])));
        if (szout[0] <= 0) {
...
template<class T>
bool downsample3dvol(Vol3DSimple<T>* &outimg, Vol3DSimple<T>* inimg, double* dfactor, unsigned char tag) {
    // check arguments
